...
// Índice de búsqueda: se construye una sola vez y permite buscar por
// nombre, alias o código sin depender de mayúsculas, tildes o guiones.
const stationSearchIndex=stations.map(st=>({
  st,
  code:String(st.code??"").trim(),
  name:stationNorm(st.name),
  aliases:(st.aliases||[]).map(stationNorm).filter(Boolean)
}));
...
const stationMatches=q=>{
  ...
  return stationSearchIndex.map(item=>{
    const terms=[item.name,...item.aliases];
    const compactTerms=terms.map(stationCompact);
    const codeCompact=item.code.replace(/\s+/g,"");
    let score=999;

    // Código exacto / parcial.
    if(codeCompact===raw.replace(/\s+/g,"")) score=0;
    else if(codeCompact.startsWith(raw.replace(/\s+/g,""))) score=1;

    // Nombre o alias exacto.
    else if(terms.some(t=>t===query)) score=2;

    // Nombre o alias empieza por la búsqueda.
    else if(terms.some(t=>t.startsWith(query))) score=3;

    // Nombre o alias contiene la búsqueda.
    else if(terms.some(t=>t.includes(query))) score=4;

    // Coincidencia sin espacios: PUIGCERDÀ / PUIG CERDÀ, etc.
    else if(compactTerms.some(t=>t.includes(queryCompact))) score=5;

    // Todas las palabras introducidas aparecen en nombre/alias.

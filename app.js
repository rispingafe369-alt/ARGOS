...
const stationSearchTokens=s=>stationNorm(s).split(" ").filter(Boolean);
...
const stationMatches=q=>{
  ...
  const qTokens=stationSearchTokens(query);

  return stations.map(st=>{
    const name=stationNorm(st.name);
    const nameCompact=stationCompact(name);
    const aliases=(st.aliases||[]).map(stationNorm).filter(Boolean);
    const terms=[name,...aliases];
    const compactTerms=terms.map(stationCompact);

    let score=999;

    // 1. Código exacto.
    if(st.code===raw.replace(/\s+/g,"")) score=0;

    // 2. Código que empieza por lo escrito.
    else if(st.code.startsWith(raw.replace(/\s+/g,""))) score=1;

    // 3. Nombre o alias exacto.
    else if(terms.some(t=>t===query)) score=2;

    // 4. Nombre o alias que empieza por lo escrito.
    else if(terms.some(t=>t.startsWith(query))) score=3;

    // 5. Nombre o alias que contiene lo escrito.
    else if(terms.some(t=>t.includes(query))) score=4;

    // 6. Coincidencia compactada: permite búsquedas como
    // "puig cerda" -> "puigcerda" y "la serna" -> "laserna".
    else if(compactTerms.some(t=>t.includes(queryCompact))) score=5;

    // 7. Todas las palabras introducidas aparecen en nombre/alias.
    else if(qTokens.length>1){
      const ok=qTokens.every(token=>
        terms.some(t=>t.includes(token)) ||
        compactTerms.some(t=>t.includes(token))
      );
      if(ok) score=6;
    }
...
function autocomplete(inputId,listId){
  ...
  const render=()=>{
    ...
    matches.forEach(s=>{
      ...
      o.innerHTML=`<span>${esc(s.name)}</span><small>${esc(s.code)}</small>`;
      list.appendChild(o);
    });
    list.classList.toggle("show",matches.length>0);
    list.querySelectorAll(".suggestion").forEach(o=>o.onclick=()=>{
      input.value=o.dataset.name;
      input.dataset.stationCode=o.dataset.code;
